...
function HealthBar({ score }) {
  ...
  const label = score >= 80 ? 'Sangat Baik' : score >= 60 ? 'Baik' : score >= 40 ? 'Perlu Perhatian' : 'Kritis'
  ...
  return (
    <div className={`rounded-lg border p-2.5 flex flex-col gap-1.5 ${bg}`}>
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] font-bold">{label} · {score}/100</span>
      </div>
...
function Pill({ text, variant }) {
  ...
  return (
    <span className={`inline-flex px-1.5 py-0.5 rounded text-[9px] font-semibold uppercase ${cls}`}>
      {text}
    </span>
  )
}
  ...
  const analyze = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/analyze', {
        ...
        body: JSON.stringify({ messages: buildMessages(tasks) }),
      })
        ...
        <div className="flex items-center justify-between px-3 py-2.5 border-b border-slate-100 bg-gradient-to-r from-violet-50 to-slate-50 flex-shrink-0">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[9px] text-slate-400">Powered by Groq · Llama 3.3</p>
            </div>
          </div>
          ...
          {error && (
            <div className="flex items-start gap-2 p-2.5 rounded-lg bg-red-50 border border-red-200">
              ...
              <div className="flex flex-col gap-1">
                ...
                <button
                  onClick={analyze}
                  className="text-[10px] text-red-500 underline text-left hover:text-red-700 transition"
                >
                  Coba lagi
                </button>
              </div>
            ...
            <>
              ...
              {lastRun && (
                <p className="text-[9px] text-slate-400 text-center pb-1">
                  ...
                  {' · '}
                  <button onClick={analyze} className="underline hover:text-slate-500 transition">
                    Refresh
                  </button>
                </p>
              )}
